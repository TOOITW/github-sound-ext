...
async function tick() {
  ...
  for (const repo of repos) {
    try {
      ...
      if (etagMap[repo]) {
        headers["If-None-Match"] = etagMap[repo];
        console.log("[GHSound] If-None-Match →", repo, etagMap[repo]);
      }

      console.log("[GHSound] checking", url);
      const res = await fetch(url, { headers });
      console.log('[GHSound] status →', repo, res.status);
      ...
      if (etag) {
        console.log('[GHSound] ETag ←', repo, etag);
        etagMap[repo] = etag;
